...
class StartUITest {
    ...
    @Test
    void whenFindByNameIsNotSuccessfully() {
        Output output = new StubOutput();
        Tracker tracker = new Tracker();
        String nonexistentName = "Nonexistent Name";
        Input input = new MockInput(
                new String[]{"0", nonexistentName, "1"}
        );
        List<UserAction> actions = new ArrayList<>();
        actions.add(new FindByNameAction(output));
        actions.add(new ExitAction(output));
        ...
        assertThat(output.toString()).isEqualTo(
                "Меню:" + ln
                        ...
                        + "1. Завершить программу" + ln
                        + "=== Вывод заявок по имени ===" + ln
                        + "Заявки с именем: " + nonexistentName + " не найдены." + ln
                        + "Меню:" + ln
                        + "0. Показать заявки по имени" + ln
                        + "1. Завершить программу" + ln
                        + "=== Завершение программы ===" + ln
    ...
    @Test
    void whenFindByIdIsNotSuccessfully() {
        Output output = new StubOutput();
        Tracker tracker = new Tracker();
        Item one = tracker.add(new Item("test1"));
        String nonexistentId =  String.valueOf(one.getId() + 1);
        Input input = new MockInput(
                new String[] {"0", nonexistentId, "1"}
        );
        List<UserAction> actions = new ArrayList<>();
        actions.add(new FindByIdAction(output));
        actions.add(new ExitAction(output));
